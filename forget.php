	...
	<div class="page-content">
		<div class="form-v4-content">
			<div class="form-left">
				...
				<img src="director.png" alt="" width="60%" style="transform: rotateY(180deg);position:relative;left:20%;top:10%;" >
				<a href="login.php"  target="_parent" rel="noopener noreferrer">
                    <div class="form-left-last" style="position:relative;top:10%" > 
                        <input type="submit"  name="account" class="account" value="SIGN IN">
                    </div>
                </div>
                </a>
			<form class="form-detail" action="forget.php" method="post" id="myform">
				...
				<div class="form-checkbox">
					<!-- <label class="container"><p><a href="#" class="text">Terms and Conditions</a></p>
...
<?php
	...
	if(isset($_POST['your_email']) && isset($_POST['password']) && isset($_POST['comfirm_password']))
	{
		if(!empty($_POST['your_email']) && !empty($_POST['password']) && isset($_POST['comfirm_password']))
		{
			$userid = $_POST['your_email'];
            $password = md5($_POST['password']);
			$con_password = md5($_POST['comfirm_password']);
			// $password = md5($_POST['password']);
            $query = " SELECT mailid FROM users where mailid = '$userid' ";
            // echo $query;
            $lnamee = " SELECT * FROM users where mailid = '$userid' ";
            $lname = $con->query($lnamee);
            ...
            if($result = mysqli_query( $con , $query))
            {
                if(mysqli_num_rows($result) != NULL)
				{
					...
					{
                        $query_forget = " UPDATE `users` SET password='$password' WHERE mailid ='$userid'  ";
						if( $query_forget_run = mysqli_query( $con, $query_forget ) )
						{
                            while($row = $lname->fetch_assoc()){
							echo "<h1 align=\"center\" style ='position: absolute;top: 0%;' ><font color=\"lime\">You have successfully changed password - ".$row['lname']."!</h1><br>";
                            }
						}
						else
						{
                ...
                // else
                // {
                //     echo "<h3 style ='position: absolute;top: 0%;' ><font color=\"red\">* 3..Can't perform the operation now.Please try again later.</font></h3>";
                // }
            }
            else
            {
